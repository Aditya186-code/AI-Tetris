...
class Tetris:
    ...
    def render(self):
        ...
        # img = img.resize((Tetris.BOARD_WIDTH * 40, Tetris.BOARD_HEIGHT * 30))
        img = self.image_resize(np.array(img), Tetris.BOARD_WIDTH * 30, Tetris.BOARD_HEIGHT * 30 )
        # cv2.imshow("Another", np.array(img))
        img = np.array(img)
